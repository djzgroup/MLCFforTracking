function [disp_row, disp_col, sind, max_scale_response] = resp_newton(response, responsef, iterations, ky, kx, use_sz)

[max_resp_row, max_row] = max(response, [], 1);
[init_max_response, max_col] = max(max_resp_row, [], 2);
[max_response, max_sind] = max(init_max_response, [], 3);
max_num = response(max_row(1,max_col(1,max_sind),max_sind),max_col(1,max_sind),max_sind);
%[max_row(1,max_col(1,max_sind),max_sind),max_col(1,max_sind),max_sind max_num]
%[disp_row, disp_col, sind, max_scale_response;max_row(1,max_col(1,max_sind),max_sind),max_col(1,max_sind),max_sind max_num]
...
[a,b]=max(init_max_response,[],3);

%disp([trans_row(b),trans_col(b),b,a;disp_row, disp_col, sind,max_scale_response]);
